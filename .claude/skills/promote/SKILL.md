...
name: promote
description: |
  vault의 결정에서 파생되는 코드 종속 문서(기능 명세·TC·API 설계)를 모노레포(oursymbol) docs/ 에 만드는 PR을 연다.
  Keywords: "승격", "명세로 정리해줘", "코드로 만들어야", "모노레포에 올려줘", "promote"
---
...
vault의 결정이 **코드 사양으로 바뀔 때**, 그로부터 파생되는 문서를 모노레포 `docs/`에
만든다. 코드와 함께 움직여야 하는 문서는 코드 옆에 있어야 하기 때문이다.

> **⚠️ 이 스킬은 "문서 이사"가 아니다.**
> vault 원문(회의록·결정사항)은 **영구히 vault에 남는다.** 옮기지 않는다.
> 옵시디언은 회사의 기억 창고이고, 확정됐다고 기억에서 빼내면 창고가 아니게 된다.
> 경계는 *확정 전/후*가 아니라 **"코드가 바뀌면 같이 바뀌어야 하는가"** 다.
> — 260803 결정 #13(옵시디언=메모리), 260728 결정 #4·#5

## 언제 사용하는가?

- "이건 코드로 만들어야 해" / "명세로 정리해줘"
- 결정이 기능 명세·TC·API 설계 등 **코드에 종속된 산출물**을 필요로 할 때
- 예: 260803 결정 #4 *"TC를 별도 문서로 작성한다"* → 결정문은 vault에 남고,
  TC 문서는 코드와 함께 움직이므로 모노레포로

## 언제 쓰지 않는가

**대부분의 결정은 승격 대상이 아니다.** 회의 결정의 다수는 코드 사양이 아니라 맥락이고,
맥락은 vault에 남는 것이 맞다. **승격 건수가 적은 것은 정상이며 실패 신호가 아니다.**

"확정됐으니 옮기자"는 요청을 받으면, **코드 종속 산출물이 실제로 필요한지 먼저 되묻는다.**
필요 없으면 vault에 그대로 두는 것이 옳다고 안내한다.

## 승격하지 않는 것

| 대상 | 이유 |
|---|---|
| 회의 전사본·정리본·결정사항 | **vault의 영구 자산.** 회의 원문은 Git에 두지 않는다 (모노레포 CLAUDE.md 가드 5 주석) |
| 세일즈 딜 맥락, 파트너십, 브랜드 | 코드와 무관한 회사 맥락 — vault에 남는다 |
| 아직 보류·미결인 항목 | 사양이 확정되지 않았으므로 명세를 만들 수 없다 |
| 바이너리 | Drive에 두고 링크만 |
...
vault 원문 frontmatter에 추가한다. **원문은 제자리에 그대로 두고 링크만 붙인다.**
이게 있어야 "이 결정에서 파생된 명세가 저기 있다"를 나중에 따라갈 수 있다.
