...
대신 GROUPS(지역·섹터·스타일 19개 ETF, 2026-09-11 섹터 5→11개로 확장)는 같은
티커가 수년간 유지돼 왔으므로,
"매달 위험조정 점수(변동성 대비 초과수익) 상위를 상관필터로 5개 골라 역변동성
가중으로 다음 한 달 보유한다"는 build_book() 의 **방법론 자체**는 이 19개로
...
🔴 **이 결과로 파라미터를 고르지 말 것 (2026-09-13)**
표본이 12회뿐이라 **창 정렬 노이즈가 신호보다 크다.** 3년치를 받아 2년 창을
3거래일씩 밀며 21가지 정렬로 재보니 같은 전략·같은 데이터인데 적중률이
33~67%, 회당평균초과가 -0.14~+1.17%p 를 오갔다. 실제로 9/11 에 42%(+0.45%p)
였던 값이 9/13 에 17%(+0.09%p) 가 됐는데 시장이 아니라 period="2y" 창이 이틀
미끄러진 것뿐이다 — fetch_universe() 가 상대 기간으로 받으므로 실행일이 밀리면
창의 시작점도 밀리고, starts 그리드가 앵커된 인덱스(LOOKBACK+REBAL_DAYS)가
가리키는 **날짜**가 통째로 바뀐다.

그래서:
  - 브리핑의 [백테스트] 한 줄은 "이 규칙이 대충 되긴 하나" 정도로만 읽는다.
    주 단위 변화를 전략의 좋아짐/나빠짐으로 읽으면 안 된다.
  - VOL_PENALTY 같은 파라미터를 이 숫자로 고른 적이 있는데(2026-09-11) 단일
    정렬 과적합이었다. 비교가 필요하면 정렬 하나가 아니라 **여러 정렬의 평균**을
    봐야 한다(그러면 21배 무거워진다 — 그래서 상시로는 안 돌린다).
