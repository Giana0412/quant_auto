...
3. ${CONCLUSION_PATH} 를 작성한다. **텔레그램 메시지로 그대로 보낼 것이므로 길게 쓰지 않는다** (전체 15줄 이내, 문어체 vault 문서가 아니라 폰 알림으로 읽기 좋은 간결한 톤). 텔레그램 Markdown 문법만 쓴다 (*bold*, \`code\` — #, ## 같은 헤더 문법은 지원 안 되니 쓰지 않는다).
