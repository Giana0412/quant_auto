...
2. 다음을 Bash로 실행한다. 티커는 공백으로 구분해 뒤에 붙인다 (없으면 인자 없이):
   ...
   출력 절: [레짐] [지역] [섹터] [스타일] [종목] [스크리닝] [브레드스] [실적발표] [포지션 사이징] [스트레치] [상관]
...
3. personal/10-market/data/${TODAY_KST}-시장데이터.md 를 작성한다.
   **파일이 오늘 날짜로 이미 있어도 Edit 으로 일부만 고치지 말고, Write 로 아래
   ...
   ## 모델 포트폴리오
   ([포지션 사이징] 절을 표로: | 티커 | 비중 | 소속 |. 그 아래에 북 베타·추적오차·분산비율을
    한 줄로. **추적오차는 대회 평가축(벤치 대비 상대수익) 그 자체가 얼마나 흔들릴 수 있는지다**
    라고 짚는다. 제외된 종목이 있으면 어떤 종목과 상관이 높아서 빠졌는지 적는다.
    '과거 관측치이지 예측이 아니다'를 반드시 남긴다)
